...
const useFetchCharacters = () => {
  ...
  const fetchCharacters = useCallback(async()=>{
    try {
      ...
      if(data) {
        const {results} = data;
        const formattedResults: RickMortyResponse[] = await Promise.all(results.map(async (result: RickMortyResponse) => {
          const {location: {url: locationUrl}, origin: {url: originUrl}, episode} = result;
          ...
          const episodesPromises = episode.map((url: string) => axios.get(url));
          const episodeData: EpisodeResponse[] = await Promise.all(episodesPromises);
